...
document.querySelector("#app-prompt").addEventListener("submit", async (e) => {
  ...
  const apiPrompt = $submit.textContent === "Improve"
    ? `${originalPrompt}\n\nProvide a detailed explanation of improvements made since the last version, organized with categorized bullet points. Use emojis in the explanation to make each point more engaging and visually distinct. At the end, include a one-line summary of what changed since the previous version, enclosed in <summary> tags. Do not include any other content outside the tags.`
    : originalPrompt;
